...
namespace RDRN_Core.Streamer
{
    public partial class SyncCollector
    {
        ...
        {
            bool aiming = player.IsSubtaskActive(ESubtask.AIMED_SHOOTING_ON_FOOT) || player.IsSubtaskActive(ESubtask.AIMING_THROWABLE); // Game.IsControlPressed(0,RDR2.Control.Aim);
            ...
            if (obj.WeaponHash != null && !WeaponDataProvider.IsWeaponAutomatic(unchecked((Shared.WeaponHash)obj.WeaponHash.Value)))
            {
            ...
            if (sendShootingPacket && !_lastShooting && DateTime.Now.Subtract(_lastShot).TotalMilliseconds > 50)
            {
                ...
                {
                    bin = PacketOptimization.WriteBulletSync(0, true, aimCoord);
            ...
            else if (!sendShootingPacket && !_sent && DateTime.Now.Subtract(_lastShot).TotalMilliseconds > 50)
            {
                ...
                byte[] bin = PacketOptimization.WriteBulletSync(0, false, aimCoord);
                msg.Write((byte)PacketType.BulletSync);
