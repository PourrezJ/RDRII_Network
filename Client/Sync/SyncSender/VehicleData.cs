...
namespace RDRN_Core.Streamer
{
    public partial class SyncCollector
    {
        private static void VehicleData(Ped player)
        {
            ...
            if (Game.IsEnabledControlPressed(0,Control.VehBrake) || Game.IsEnabledControlPressed(0,Control.VehHandbrake))
            ...
            {
                obj.Flag |= (byte)VehicleDataFlags.HasAimData;
                obj.AimCoords = new Vector3(0, 0, 0);
                obj.WeaponHash = Main.GetCurrentVehicleWeaponHash(player);
                if (Game.IsEnabledControlPressed(0,Control.VehFlyAttack))
            ...
            else if (WeaponDataProvider.DoesVehicleSeatHaveGunPosition((VehicleHash)veh.Model.Hash, Util.Util.GetPedSeat(player)))
            {
                obj.Flag |= (byte)VehicleDataFlags.HasAimData;
                obj.WeaponHash = 0;
                obj.AimCoords = Main.RaycastEverything(new Vector2(0, 0));
                if (Game.IsEnabledControlPressed(0,Control.VehAttack))
                    obj.Flag |= (byte)VehicleDataFlags.Shooting;
            }
            else
            {
