    ...
    public partial class SyncPed
    {
            ...
            if (Character == null || !Character.Exists() || Character.Model.Hash != ModelHash)
            {
              ...
              //  Character.FiringPattern = FiringPattern.FullAuto;
               ...
               //Function.Call(Hash.SET_CAN_ATTACK_FRIENDLY, Character, true, true);
                ...
                JavascriptHook.InvokeStreamInEvent(new LocalHandle(Character.Handle), (int) Shared.EntityType.Player);
                return true;
            }
        ...
        private void UpdateOnFootAnimation()
        {
            ...
            if (_playingGetupAnim)
            {
                ...
                {
                    //UpdatePlayerPedPos();
                    var currentTime = Function.Call<float>(Hash._GET_ENTITY_ANIM_CURRENT_TIME, Character, getupAnim[0], getupAnim[1]);
            ...
            if (!IsCustomAnimationPlaying)
            {
                if (!IsReloading)
                {
                    ...
                    {
                        var currentTime = Function.Call<float>(Hash._GET_ENTITY_ANIM_CURRENT_TIME, Character, lastMeleeAnim.Split()[0], lastMeleeAnim.Split()[1]);
        ...
        {
            var currentTime = Function.Call<float>(Hash._GET_ENTITY_ANIM_CURRENT_TIME, Character,
            ...
            if (!meleeSwingDone && CurrentWeapon != unchecked((int)WeaponHash.Unarmed))
            {
                ...
                if (gunEntity != null)
                {
                    ...
                    var ray = World.Raycast(start, end, (int)Math.Abs(end.X - start.X), IntersectOptions.Peds1, Character);
        ...
        private void DisplayMeleeAnimation(int hands)
        {
            ...
            /*
            Character.Quaternion = RDR2.Math.Quaternion.Slerp(Character.Quaternion, _rotation.ToQuaternion(),
            ...
            else
            {
                ...
                if (_entityToAimAt != null && _entityToAimAt.Exists() && _entityToWalkTo != null && _entityToWalkTo.Exists() && Character.Exists())
                {
                    if (!Character.IsSubtaskActive(ESubtask.AIMING_GUN))
                    {
                        if (OnFootSpeed > 0)
                        {
                            Function.Call(Hash.TASK_GO_TO_ENTITY_WHILE_AIMING_AT_ENTITY, Character, _entityToWalkTo, _entityToAimAt, (float)OnFootSpeed, false, 10000, 10000, true, true, (uint)FiringPattern.Default);
        ...
        private void DisplayCustomAnimation()
        {
            ...
            if (!IsCustomScenarioPlaying)
            {
                ...
                if (Function.Call<bool>(Hash.IS_ENTITY_PLAYING_ANIM, Character,
                    ...
                    Function.Call<float>(Hash._GET_ENTITY_ANIM_TOTAL_TIME, Character, CustomAnimationDictionary,
            ...
            {
                Function.Call(Hash._TASK_START_SCENARIO_IN_PLACE, Character, CustomAnimationName, 0, 0);
        ...
        private void UpdatePosition(bool updatePosition = true, bool updateRotation = true, bool updateVelocity = true)
        {
           ...
           // if (updateRotation) Character.Quaternion = RDR2.Math.Quaternion.Lerp(Character.Quaternion, Rotation.ToQuaternion(), 0.10f);
