    ...
    public partial class SyncPed
    {
        private bool CreateVehicle()
        {
            ...
            if (_isInVehicle && MainVehicle != null && Character.IsInVehicle(MainVehicle) && playerChar.IsInVehicle(MainVehicle) && VehicleSeat == -1 && Function.Call<int>(Hash.GET_SEAT_PED_IS_TRYING_TO_ENTER, playerChar) == -1 && Util.Util.GetPedSeat(playerChar) == 0)
            {
                ...
                playerChar.Task.WarpIntoVehicle(MainVehicle, (int)RDRN_Core.VehicleSeat.Driver);
        ...
        private void DisplayVehiclePosition()
        {
            if (_lastPosition != null)
            {
                ...
                {
                    MainVehicle.Quaternion = Shared.Math.Quaternion.Slerp(_lastVehicleRotation.ToQuaternion(), _vehicleRotation.ToQuaternion(), avrLat);
                }
                else
                {
        ...
        private void EnterVehicle()
        {
            if (EnteringVehicle)
            {
                ...
                while (Character.IsSubtaskActive(ESubtask.ENTERING_VEHICLE_GENERAL) || Character.IsSubtaskActive(ESubtask.ENTERING_VEHICLE_ENTERING))
                {
                    Script.Yield();
                    Script.Wait(1500);
                    Character.SetIntoVehicle(MainVehicle, VehicleSeat);
                }
            }
            else
            {
        ...
        private bool DisplayVehicleDriveBy()
        {
            ...
            /*
            else if ((VehicleHash) VehicleHash == RDR2.VehicleHash.Savage || (VehicleHash) VehicleHash == RDR2.VehicleHash.Hydra || (VehicleHash) VehicleHash == RDR2.VehicleHash.Lazer)
            {
        ...
        private void UpdateVehicleMountedWeapon()
        {
            ...
            {
                var delay = 30;
                //if ((VehicleHash) VehicleHash == RDR2.Native.VehicleHash.Rhino) delay = 300;
                ...
                {
                    /*
                    if (((VehicleHash)VehicleHash == RDR2.VehicleHash.Rhino &&
                         DateTime.Now.Subtract(_lastRocketshot).TotalMilliseconds > 1000) ||
                        ((VehicleHash)VehicleHash != RDR2.VehicleHash.Rhino))
                    {
                        _lastRocketshot = DateTime.Now;
            ...
            else if (!WeaponDataProvider.DoesVehicleSeatHaveMountedGuns((VehicleHash)VehicleHash) || VehicleSeat != -1)
            {
                ...
                if (IsShooting || IsAiming)
                {
                    if (!_lastDrivebyShooting)
                    {
                        ...
                        Function.Call(Hash.TASK_DRIVE_BY, Character, 0, 0, AimCoords.X, AimCoords.Y, AimCoords.Z,
                            0, 0, 0, unchecked((int)FiringPattern.Default));
                    }
                    else
                    {
                ...
                if (!IsShooting && !IsAiming && _lastDrivebyShooting && Game.GameTime - _lastVehicleAimUpdate > 200)
                {
                    ...
                    task.ClearLookAt();
                    //RDR2.UI.Screen.ShowNotification("Done shooting");
                    //RDR2.UI.Screen.ShowSubtitle("Done Shooting1", 300);
