...
namespace RDRN_Core
{
    public partial class Main
    {
        ...
        */
        ...
        private static bool _hasScAvatar;

        private static bool ListSorting = true;
       ...
       // private readonly MenuPool _menuPool;
        ...
        private static int _lastBytesSent;
        private static int _lastBytesReceived;
            ...
            ThreadPool.QueueUserWorkItem((WaitCallback)delegate
            {
                try
                {
                    using (var wc = new ImpatientWebClient())
                    {
                        ...
                        */
                        LogManager.WriteLog("Set text to " + jsonObj.Message + " and title to " + jsonObj.Title);
                    }
                }
                catch (WebException)
                {
        ...
        private void RebuildServerBrowser()
        {
            ...
            ThreadPool.QueueUserWorkItem(delegate
            {
                try
                {
                    ...
                    //const string masterServerAddress = "http://master.gtanet.work";
                    const string masterServerAddress = "http://google.com";

                    LogManager.WriteLog("Contacting " + masterServerAddress);
                    ...
                    catch (Exception e)
                    {
                        ...
                        logOutput += "\n";
                        LogManager.WriteLog("masterserver: " + logOutput);
                    }
                ...
                {
                    LogManager.Exception(e, "DISCOVERY CRASH");
                }
                finished = true;
            });
            ...
            #region Settings
            {
                ...
                var GeneralMenu = new TabInteractiveListItem("General", new List<UIMenuItem>());
                {
                    ...
                    nameItem.Activated += (sender, item) =>
                    {
                        if (IsOnServer())
                        {
                            RDR2.UI.Screen.ShowNotification("You cannot change your Display Name while connected to a server.");
                            return;
                        }
                        MainMenu.TemporarilyHidden = true;
