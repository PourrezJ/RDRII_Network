...
namespace RDRN_Core
{
    public class Events : Script
    {
        ...
        private static PedHash _lastPlayerModel = PedHash.Player_Three;
        ...
        public static void InvokeEvents(object sender, EventArgs e)
        {
            ...
            {
                JavascriptHook.InvokeCustomEvent(api => api?.invokeonLocalPlayerShoot((int)(player.Weapons.Current?.Hash ?? 0), Main.RaycastEverything(new Vector2(0, 0))));
            }
            #endregion
            ...
            if (player.CurrentVehicle != null)
            {
                ...
                if ((netPlayerCar = Main.NetEntityHandler.EntityToNet(playerCar.Handle)) != 0)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        item.Position = playerCar.Position;
                        item.Rotation = playerCar.Rotation;
                        item.Health = playerCar.Health;
                        item.IsDead = playerCar.IsDead;
                        //item.DamageModel = playerCar.GetVehicleDamageModel();

                        if (lastHealth != playerCar.Health)
                        {
