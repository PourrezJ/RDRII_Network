...
namespace RDRN_Core
{
	public struct RelationshipGroup : INativeValue, IEquatable<RelationshipGroup>
	{
		RelationshipGroup(string name) : this()
		{
			...
			unsafe
			{
				Function.Call(NativeHash.ADD_RELATIONSHIP_GROUP, name, &hashArg);
			}
		...
		{
			return Function.Call<Relationship>(NativeHash.GET_RELATIONSHIP_BETWEEN_GROUPS, Hash, targetGroup.NativeValue);
		}
		...
		public void Remove()
		{
			Function.Call(NativeHash.REMOVE_RELATIONSHIP_GROUP, Hash);
		}
