...
using Shared;
using Quaternion = Shared.Math.Quaternion;
using Vector3 = Shared.Math.Vector3;
using EntityType = Shared.EntityType;
...
namespace RDRN_Core.Streamer
{
    ...
    internal partial class Streamer
    {
        ...
        public void CreateMarker(int type, Vector3 position, Vector3 rotation, Vector3 dir, Vector3 scale, int r, int g, int b, int a, int netHandle, bool bobUpAndDown = false)
        {
        ...
        public int CreateLocalVehicle(int model, Vector3 pos, float heading)
        {
            var veh = CreateVehicle(model, pos, new Vector3(0, 0, heading), --_localHandleCounter);
            veh.LocalOnly = true;
        ...
        public int CreateLocalBlip(Vector3 pos)
        {
        ...
        public int CreateLocalPed(int model, Vector3 pos, float heading)
        {
            ...
            pp.Alpha = 255;
            pp.ModelHash = model;
            pp.Rotation = new Vector3(0, 0, heading);
            pp.Dimension = 0;
        ...
        public void UpdatePlayer(int netHandle, PlayerProperties prop)
        {
            ...
            if (prop.Position != null)
                ((SyncPed)rem).Position = prop.Position;
            if (prop.Rotation != null)
                ((SyncPed)rem).Rotation = prop.Rotation;
        ...
        public void UpdateAttachments()
        {
            ...
            for (var index = attaches.Count - 1; index >= 0; index--)
            {
                ...
                item.Position = entityTarget.Position;
                ...
                {
                    switch ((Shared.EntityType) item.EntityType)
                    {
                        ...
                        {
                            item.Position = entityTarget.GetOffsetInWorldCoords(item.AttachedTo.PositionOffset);
                            item.Rotation = entityTarget.Rotation + item.AttachedTo.RotationOffset;
                        }
                            break;
                        case Shared.EntityType.TextLabel:
                        {
                            item.Position =
                                entityTarget.GetOffsetInWorldCoords(item.AttachedTo.PositionOffset);

                            if (entityTarget.IsPed() && new Ped(entityTarget.Handle).IsInVehicle())
                            {
                                item.Position +=
                                    (new Ped(entityTarget.Handle).CurrentVehicle.Velocity / Game.FPS);
                            }
                        }
                            break;
                    }
        ...
        public void UpdateInterpolations()
        {
            ...
            foreach (var ent in ents)
            {
                if (ent.PositionMovement != null)
                {
                    ...
                    ent.Position = Vector3.Lerp(ent.PositionMovement.StartVector,
                            ...
                            case Shared.EntityType.Player:
                                {
                                    ...
                                    if (gameEnt != null) gameEnt.PositionNoOffset = ent.Position;
                                }
                                break;
                            case Shared.EntityType.Blip:
                                {
                ...
                if (ent.RotationMovement != null)
                {
                    ...
                    ent.Rotation = Vector3.Lerp(ent.RotationMovement.StartVector,
                            ...
                            case Shared.EntityType.Player:
                                {
                                    ...
                                    if (gameEnt != null)
                                    {
                                        ...
                                        gameEnt.Rotation = ent.Rotation; // Gimbal lock!
                                    }
                                }
                                break;
                        }
                ...
                case Shared.EntityType.Player:
                    {
                        ...
                        {
                            ped.StreamedIn = true;
                            JavascriptHook.InvokeStreamInEvent(new LocalHandle(ped.LocalHandle), (int)Shared.EntityType.Player);
                        }
                    }
                    break;
        ...
        public void StreamOut(IStreamedItem item)
        {
            ...
            switch ((Shared.EntityType)item.EntityType)
            {
                case Shared.EntityType.Prop:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Prop);
                ...
                case Shared.EntityType.Vehicle:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Vehicle);
                        var obj = new Prop(data.LocalHandle);
                        if (obj.Exists()) obj.Delete();
                    }
                    break;
                case Shared.EntityType.Ped:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Ped);
                        var obj = new Prop(data.LocalHandle);
                        if (obj.Exists()) obj.Delete();
                    }
                    break;
                case Shared.EntityType.Blip:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Blip);
                ...
                case Shared.EntityType.Pickup:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Pickup);
                ...
                case Shared.EntityType.Particle:
                    {
                        var data = (ILocalHandleable)item;
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Particle);
                ...
                case Shared.EntityType.Player:
                    ...
                    if (ped != null)
                    {
                        JavascriptHook.InvokeStreamOutEvent(new LocalHandle(ped.Character?.Handle ?? 0), (int)Shared.EntityType.Player);
                        ped.Clear(); //TODO
                    }
                    break;
            }
        ...
        private void StreamInVehicle(RemoteVehicle data)
        {
            ...
            if (data.Alpha < 255) veh.Alpha = data.Alpha;
        ...
        private void StreamInBlip(RemoteBlip item)
        {
            ...
            ourBlip.Scale = new Vector2(item.Scale, item.Scale);
            ...
            //JavascriptHook.InvokeStreamInEvent(new LocalHandle(ourBlip.Handle), (int)RDR2NetworkShared.EntityType.Blip);
        }
        ...
        private void StreamInParticle(RemoteParticle data)
        {
            ...
            data.StreamedIn = true;
            JavascriptHook.InvokeStreamInEvent(new LocalHandle(data.LocalHandle), (int)Shared.EntityType.Particle);
        }
        ...
        private static void StreamInPickup(RemotePickup pickup)
        {
            ...
            JavascriptHook.InvokeStreamInEvent(new LocalHandle(newPickup), (int)Shared.EntityType.Pickup);
        }
        ...
        private static void StreamInProp(RemoteProp data)
        {
            ...
            {
                ourProp.Rotation = data.Rotation;
            }

            //if (PacketOptimization.CheckBit(data.Flag, EntityFlag.Collisionless)) ourProp.IsCollisionEnabled = false;
            if (data.Alpha < 255) ourProp.Alpha = data.Alpha;
            ourProp.FreezePosition = true;
            ...
            JavascriptHook.InvokeStreamInEvent(new LocalHandle(ourProp.Handle), (int)Shared.EntityType.Prop);
        }
        ...
        private static void StreamInPed(RemotePed data)
        {
            ...
            Ped ped = null;
            if (model.IsLoaded) ped = World.CreatePed(model, data.Position, data.Rotation.Z);
            ...
            ped.PositionNoOffset = data.Position;
        ...
        public void DrawMarkers()
        {
            ...
            for (var index = markers.Count - 1; index >= 0; index--)
            {
                ...
                World.DrawMarker((MarkerType)marker.MarkerType, marker.Position, marker.Direction, marker.Rotation, marker.Scale, Color.FromArgb(marker.Alpha, marker.Red, marker.Green, marker.Blue), marker.BobUpAndDown);
            }
        ...
        public void DrawLabels()
        {
            ...
            for (var index = labels.Count - 1; index >= 0; index--)
            {
                ...
                DrawLabel3D(origin, label.Text, label.Position, label.Range, label.Size, Color.FromArgb(label.Alpha, label.Red, label.Green, label.Blue), label.EntitySeethrough);
            }
        }
