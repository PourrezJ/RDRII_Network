...
using VehicleHash = Shared.VehicleHash;
using WeaponHash = Shared.WeaponHash;
using System.ComponentModel;
using RDRN_Core.Gui.Cef;
using Shared.Math;
//using WeaponComponent = GTANetwork.Util.WeaponComponent;
//using WeaponComponent = RDR2.WeaponComponentHash;
...
namespace RDRN_Core.Javascript
{
    ...
    public class JavascriptHook : Script
    {
       ...
       // internal static List<UIResText> TextElements { get; set; }
        ...
        private static void LogException(Exception ex)
        {
            ...
            LogManager.Exception(ex, "CLIENTSIDE SCRIPT ERROR");
        }
    }
    ...
    public class ScriptContext
    {
        ...
        public PointF getCursorPositionMantainRatio()
        {
            ...
            var mouseX = Function.Call<float>(Hash.GET_DISABLED_CONTROL_NORMAL, 0, (int)RDR2.Control.CursorX) * res.Width;
            var mouseY = Function.Call<float>(Hash.GET_DISABLED_CONTROL_NORMAL, 0, (int)RDR2.Control.CursorY) * res.Height;
        ...
        */
        public Vector3 screenToWorld(PointF pos, Vector3 camPos, Vector3 camRot) // TODO: replace this with a camera object
        {
            ...
            var p = Main.RaycastEverything(norm2, camPos, camRot);
        ...
        */
        public class Raycast
        {
            ...
            public Vector3 hitCoords => wrapper.HitPosition;
        }

        public Raycast createRaycast(Vector3 start, Vector3 end, int flag, LocalHandle? ignoreEntity)
        {
            return ignoreEntity != null ? new Raycast(World.Raycast(start, end, (IntersectOptions) flag, new Prop(ignoreEntity.Value.Value))) : new Raycast(World.Raycast(start, end, (IntersectOptions)flag));
        }
        ...
        public Browser createCefBrowser(string url, double width, double height, bool local = true, bool focus = true)
        {
#if RELATIVE_CEF_POS
            ...
            var newBrowser = new Browser(Engine, url, new Size(w, h), local);
        ...
        public void destroyCefBrowser(Browser browser)
        {
            ...
            {
                LogManager.Exception(ex, "DESTROYCEFBROWSER");
            }
        }
...
//        }
        ...
        public Vector3 getEntityPosition(LocalHandle entity)
        {
            if (entity.Properties<IStreamedItem>() == null || entity.Properties<IStreamedItem>().StreamedIn)
                return new Prop(entity.Value).Position;
        ...
        */
        ...
        {
            return player.Value == Game.Player.Character.Handle ? Main.RaycastEverything(new Vector2(0, 0)) : findPlayer(player).AimCoords;
        }
        ...
        */
        ...
        {
            Function.Call(Hash._TASK_START_SCENARIO_IN_PLACE, Game.Player.Character, name, 0, 0);
        }*/
        ...
        */
        ...
        {
            new Prop(entity.Value).FreezePosition = frozen;
        }

        public void setEntityVelocity(LocalHandle entity, Vector3 velocity)
        {
            new Prop(entity.Value).Velocity = velocity;
        }
        ...
        {
            Game.Player.Character.Weapons[(RDR2.WeaponHash)weapon].SetComponent((WeaponComponent) component, true);
        }

        public void removePlayerWeaponComponent(int weapon, int component)
        {
            Game.Player.Character.Weapons[(RDR2.WeaponHash)weapon].SetComponent((WeaponComponent)component, false);
        }

        public bool hasPlayerWeaponComponent(int weapon, int component)
        {
            return Game.Player.Character.Weapons[(RDR2.WeaponHash) weapon].IsComponentActive((WeaponComponent) component);
        }
        ...
        */
        ...
        public void setEntityPosition(LocalHandle ent, Vector3 pos, bool ground = false)
        {
            ...
            if (ground)
            {
                if (handle > 0) new Prop(handle).Position = new RDR2.Math.Vector3(pos.X, pos.Y, getGroundHeight(pos));
        ...
        {
            var handle = ent.Value;
            if (handle > 0) new Prop(handle).Rotation = rot;
        ...
        {
            return new Prop(entity.Value).GetOffsetInWorldCoords(offset);
        }

        public Vector3 getOffsetFromWorldCoords(LocalHandle entity, Vector3 pos)
        {
            return new Prop(entity.Value).GetOffsetFromWorldCoords(pos);
        }
        /*
        ...
        {
            NativeUI.BigMessageThread.MessageInstance.ShowWeaponPurchasedMessage(text, weaponName, (RDR2.WeaponHash) weapon, time);
        }*/
        ...
        */
        ...
        {
            RDRN_Core.UI.Screen.ShowSubtitle(text);
        }
        /*
        public void displaySubtitle(string text, double duration)
        {
            RDR2.UI.Screen.ShowSubtitle(text, (int) duration);
        }
        */
        ...
        {
            if (player == getLocalPlayer()) return Main.RaycastEverything(new Vector2(0, 0));

            var opp = findPlayer(player);
            return opp != null ? opp.AimCoords : new Vector3();
        }
        ...
        */
        ...
        {
            if (blip.Properties<IStreamedItem>() == null || blip.Properties<RemoteBlip>().StreamedIn)
                new Blip(blip.Value).Scale = new Vector2((float)scale, (float)scale);
        ...
        */
        ...
        {
            return new LocalHandle(Main.NetEntityHandler.CreateLocalMarker(markerType, pos, dir, rot, scale, alpha, r, g, b, 0, bobUpAndDown), HandleType.LocalHandle);
        }
        ...
        {
            return new LocalHandle(Main.NetEntityHandler.CreateLocalLabel(text, pos, (float)range, (float)size, entitySeethrough, 0), HandleType.LocalHandle);
        }
        ...
        public UIResText addTextElement(string caption, double x, double y, double scale, int r, int g, int b, int a, int font, int alignment)
        {
            var txt = new UIResText(caption, new Point((int) x, (int) y), (float) scale, Color.FromArgb(a, r, g, b),
                (RDR2.UI.Font) font, (UIResText.Alignment) alignment);
...
#region Menus
        ...
        */
        ...
        {
            return Game.IsControlPressed(0,(RDRN_Core.Control)control);
        }

        public bool isDisabledControlJustReleased(int control)
        {
            return Game.IsDisabledControlJustReleased(0, (RDRN_Core.Control)control);
        }

        public bool isDisabledControlJustPressed(int control)
        {
            return Game.IsDisabledControlJustPressed(0, (RDRN_Core.Control)control);
        }
        ...
        {
            return Game.IsControlJustReleased(0,(RDRN_Core.Control)control);
        }

        public void disableControlThisFrame(int control)
        {
            Game.DisableControlThisFrame(0, (RDRN_Core.Control)control);
        }

        public void EnableControlThisFrame(int control)
        {
            Game.EnableControlThisFrame(0,(RDRN_Core.Control)control);
        }
        ...
        {
            return Game.GetControlNormal(0, (RDRN_Core.Control) control);
        }

        public float getDisabledControlNormal(int control)
        {
            return Game.GetDisabledControlNormal(0 ,(RDRN_Core.Control)control);
        }

        public void setControlNormal(int control, double value)
        {
            Game.SetControlNormal(0 ,(RDRN_Core.Control)control, (float)value);
        }
        /*
        ...
        */
        ...
        //    Main.PlayerChar.Weapons.Give((RDR2.WeaponHash) weapon, ammo, equipNow, ammoLoaded);
        //}
        ...
        {
            return Game.Player.Character.Weapons.HasWeapon((uint)weapon);
        }
        ...
        public void setWeather(int weather)
        {
            if (weather >= 0 && weather < Enums._weather.Length)
                Function.Call(Hash._SET_WEATHER_TYPE, Enums._weather[weather]);
        }
        ...
        {
            Function.Call(Hash._SET_WEATHER_TYPE, Main.Weather);
        }
