...
namespace RDRN_Core.Util
{
    ...
    public static class Util
    {
        ...
        internal static void SaveSettings(object p)
        {
        ...
        {
            if (model == (int) Shared.WeaponHash.Unarmed ||
                model == 0) return;
        ...
        public static void SafeNotify(string msg)
        {
            if (!string.IsNullOrWhiteSpace(msg))
            {
                try
                {
                    //RDR2.UI.Screen.ShowNotification(msg);
                }
                catch (Exception)
                {
                    // ignored
...
/*
        ...
        */
        public static bool IsVehicleEmpty(Vehicle veh)
        {
            ...
            if (!veh.IsSeatFree((int)VehicleSeat.Driver)) return false;
            for (int i = 0; i < veh.PassengerCount; i++)
            {
                if (!veh.IsSeatFree(i))
                    return false;
        ...
        public static int GetPedSeat(Ped ped)
        {
            ...
            for (int i = 0; i < vehicle.PassengerCount; i++)
            {
    ...
    public static class VectorExtensions
    {
        ...
        public static float LengthSquared(this Vector3 left)
        {
            return left.X * left.X + left.Y * left.Y + left.Z + left.Z;
        }

        public static float Length(this Vector3 left)
        {
            return (float)Math.Sqrt(left.LengthSquared());
        }

        public static Vector3 Sub(this Vector3 left, Vector3 right)
        {
