...
namespace RDRN_Core
{
	...
	internal class ConsoleScript : Script
	{
		...
		public ConsoleScript()
		{
			...
			ToggleKey = Keys.F3;
			PageDownKey = Keys.PageDown;
			PageUpKey = Keys.PageUp;
		}
		...
		internal void OnTick(object sender, EventArgs e)
		{
			...
			if (_lastClosed > now)
			{
				if (Native.Function.Call<bool>(Native.Hash.IS_CONTROL_ENABLED, 2))
		...
		private void DrawRect(float x, float y, int width, int height, Color color)
		{
			...
			Native.Function.Call(Native.Hash.DRAW_RECT,
				(x / WIDTH) + w * 0.5f,
				(y / HEIGHT) + h * 0.5f,
				w, h,
		...
		private void DrawText(float x, float y, string text, float scale, int font, Color color)
		{
			...
			var res = Function.Call<string>(Hash._CREATE_VAR_STRING, 10, "LITERAL_STRING", text);
			Function.Call(Hash._DISPLAY_TEXT, res, xNew, yNew);
		}
