...
namespace RDRN_Core
{
	public sealed class Weapon
	{
		...
		public bool IsPresent => Hash == WeaponHash.Unarmed || Function.Call<bool>(NativeHash.HAS_PED_GOT_WEAPON, Owner.Handle, (uint)Hash, false, false);

		public Model Model => GetModelByHash(Hash);

		public WeaponGroup Group => Function.Call<WeaponGroup>(NativeHash.GET_WEAPONTYPE_GROUP, (uint)Hash);

		public AmmoType AmmoType => Function.Call<AmmoType>(NativeHash.GET_PED_AMMO_TYPE_FROM_WEAPON, Owner.Handle, (uint)Hash);

		public int Ammo
		{
			get => Hash == WeaponHash.Unarmed
				? 1
				: Function.Call<int>(NativeHash.GET_PED_AMMO_BY_TYPE, Owner.Handle, (uint)AmmoType);
			set => Function.Call(NativeHash.SET_PED_AMMO_BY_TYPE, Owner.Handle, (uint)AmmoType, value);
		}
		...
		public int AmmoInClip
		{
			get {
				...
				unsafe
				{
					if (Function.Call<bool>(NativeHash.GET_AMMO_IN_CLIP, Owner.Handle, (uint)Hash, &ammo))
					{
						return ammo;
					}
				}
			...
			set {
				...
				if (IsPresent)
				{
					Function.Call(NativeHash.SET_AMMO_IN_CLIP, Owner.Handle, (uint)Hash, value);
				}
				else
				{
		...
		public int MaxAmmo
		{
			get {
				...
				unsafe
				{
					return Function.Call<bool>(NativeHash.GET_MAX_AMMO, Owner.Handle, (uint)Hash, &ammo) ? ammo : 1;
				}
			}
		}

		public int MaxAmmoInClip => Hash == WeaponHash.Unarmed ? 1 : Function.Call<int>(NativeHash.GET_MAX_AMMO_IN_CLIP);

		public bool InfiniteAmmo
		{
			set => Function.Call(NativeHash.SET_PED_INFINITE_AMMO, Owner.Handle, (uint)Hash, value);
		}

		public void Remove()
		{
			Function.Call(NativeHash.REMOVE_WEAPON_FROM_PED, Owner.Handle, (uint)Hash, false, false);
		}
