...
System::UInt64 EncodeObject(System::Object^ obj)
{
	...
	if (type == RDRN_Module::Native::InputArgument::typeid) {
		return static_cast<RDRN_Module::Native::InputArgument^>(obj)->_data;

	} else if (type == RDRN_Module::Native::OutputArgument::typeid) {
		return System::IntPtr(static_cast<RDRN_Module::Native::OutputArgument^>(obj)->GetPointer()).ToInt64();

	} else if (RDRN_Module::Native::INativeValue::typeid->IsAssignableFrom(type)) {
		return static_cast<RDRN_Module::Native::INativeValue^>(obj)->NativeValue;
		...
		return RDRN_Module::Native::Function::AddStringPool(static_cast<System::String^>(obj)).ToInt64();

	} else if (type == System::IntPtr::typeid) {
		return static_cast<System::IntPtr>(obj).ToInt64();
	}
	else if (IHandleable::typeid->IsAssignableFrom(type)) {
		return safe_cast<IHandleable^>(obj)->Handle;
	}


	RDRN_Module::LogManager::WriteLog("*** Tried encoding unhandled type {0}:\n{1}", type->FullName, System::Environment::StackTrace);
	return 0;
}
...
System::Object^ DecodeObject(System::Type^ type, System::UInt64* value)
{
	...
	} else if (type == RDRN_Module::Math::Vector2::typeid) {
		const auto data = reinterpret_cast<const float*>(value);
		return gcnew RDRN_Module::Math::Vector2(data[0], data[2]);

	} else if (type == RDRN_Module::Math::Vector3::typeid) {
		const auto data = reinterpret_cast<const float*>(value);
		return gcnew RDRN_Module::Math::Vector3(data[0], data[2], data[4]);

	} else if (RDRN_Module::Native::INativeValue::typeid->IsAssignableFrom(type)) {
		// Warning: Requires classes implementing 'INativeValue' to repeat all constructor work in the setter of 'NativeValue'
		auto ret = static_cast<RDRN_Module::Native::INativeValue^>(System::Runtime::Serialization::FormatterServices::GetUninitializedObject(type));
